...
from ceviche_challenges import jax_primitives as primitives
from ceviche_challenges import units as u
from ceviche_challenges.ceviche_simulate import ceviche_simulate as simulator
...
class Model(abc.ABC):
  ...
  def simulate(
    ...
    def _simulate(excite_port_idx_and_omega):
      excite_port_idx, omega = excite_port_idx_and_omega
      return excite_port_idx, omega, *simulator(excite_port_idx, omega, dl, epsilon_r_bg, pml_width, epsilon_r, self.ports)
    ...
    sparams = jnp.stack(jnp.array(sparams), axis=self.SPARAMS_INPUT_PORT_AXIS)
    efields = jnp.stack(jnp.array(efields), axis=self.FIELDS_INPUT_PORT_AXIS)
