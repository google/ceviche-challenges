...
from ceviche_challenges import jax_primitives
import numpy as np
...
class PrimitivesTest(absltest.TestCase):
  ...
  def test_grad_insert_design_variable(self):
    ...
    def func(x):
      y0 = jnp.square(x) - 1.0
      y1 = jax_primitives.insert_design_variable(y0, destination, coords)
      return jnp.sum(y1)

    self.assertTrue(
        np.allclose(jax.grad(func)(design_var),
  ...
  def test_insert_design_variable_invalid(self):
    ...
    with self.assertRaises(ValueError):
      ...
      coords = (-1, 4, 4, 9)
      jax_primitives.insert_design_variable(design_var, destination, coords)
    with self.assertRaises(ValueError):
      destination = np.ones((10, 10), dtype=np.float64)
      design_var = np.ones((5, 5), dtype=np.float64)
